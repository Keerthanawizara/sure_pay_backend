...
const paymentDataList = (request,h) => {
        return new Promise((resolve,reject) => 
            paymentCollection.paginate({},{offset:0, limit:10},(err,docs) => {
                if (!err) {
               resolve({status:true,message:"view payment DataList"})
                 }else{
                   reject(err)
                 }
            }))
    ...
    const paymentRecord = (req,h) => {
        ...
        const params = {_id: mongoose.Types.ObjectId(req.params.id),pin:payload.pin};  
            return new Promise((resolve,reject) => {
                paymentCollection.findById(
                    params,
                    (Data,(err,docs) => {
                        if(!err){
                           resolve({status:true,message:"get one payment record"})
                        }else{
                          reject(err)
                        }
                    })); 
